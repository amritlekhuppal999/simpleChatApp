...
function chatroomSocketHandler(chatroom_io){
    
    // IO middleware injected with session middleware
    // chatroom_io.engine.use(sessionMiddleware);
    ...
    chatroom_io.on('connection', (socket) => {
        ...
        // console.log(socket.request.session, sessions.user, socket.id, chatroom_io.sockets.adapter);
        ...
        socket.on('joinRoom', CHAT_ROOM_NAME=>{
            ...
            // inRoomUsers = chatroom_io.sockets.adapter.rooms.get(CHAT_ROOM_NAME);
            inRoomUsers = chatroom_io.adapter.rooms.get(CHAT_ROOM_NAME);
        ...
        socket.on('disconnect', () => {
            ...
            chatroom_io.emit('disconnect-message', {
                user_data: USER_DATA,
